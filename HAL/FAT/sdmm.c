...
#include "hal.h"			/* Include device specific declareation file here */

//#define DO_INIT()					/* Initialize port for MMC DO as input */
//#define DO			(GPIOB->IDR & GPIO_Pin_14)	/* Test for MMC DO ('H':true, 'L':false) */
//
////#define DI_INIT()	DDRB  |= 0x02	/* Initialize port for MMC DI as output */
//#define DI_H()		GPIOB->BSRRL = GPIO_Pin_15	/* Set MMC DI "high" */
//#define DI_L()		GPIOB->BSRRH = GPIO_Pin_15	/* Set MMC DI "low" */
//
////#define CK_INIT()	DDRB  |= 0x04	/* Initialize port for MMC SCLK as output */
//#define CK_H()		GPIOB->BSRRL = GPIO_Pin_13	/* Set MMC SCLK "high" */
//#define CK_L()		GPIOB->BSRRH = GPIO_Pin_13	/* Set MMC SCLK "low" */

//#define CS_INIT()	DDRB  |= 0x08	/* Initialize port for MMC CS as output */
#define CS_H()		GPIOB->BSRRL = GPIO_Pin_12	/* Set MMC CS "high" */
#define CS_L()		GPIOB->BSRRH = GPIO_Pin_12	/* Set MMC CS "low" */
...
DWORD get_fattime(void) {
	if (gps.FixValid) {
		return ((DWORD) (gps.date.year - 1980) << 25) /* Year 2013 */
		| ((DWORD) gps.date.month << 21) /* Month 7 */
		| ((DWORD) gps.date.day << 16) /* Mday 28 */
		| ((DWORD) gps.date.hour << 11) /* Hour 0 */
		| ((DWORD) gps.date.minute << 5) /* Min 0 */
		| ((DWORD) gps.date.second >> 1); /* Sec 0 */
	} else {
